    ...
    return (
        <div ref={setNodeRef} className={clsx("flex flex-col h-full rounded-xl transition-colors", isOver ? "bg-indigo-50/50 ring-2 ring-indigo-200" : "bg-gray-50 dark:bg-slate-800/30")}>
            ...
            <div
                onClick={onDayClick}
                className={clsx(
                    "p-3 text-center border-b border-gray-100 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800/60 rounded-t-xl transition-colors",
                    isToday && "bg-indigo-50"
                )}
            >
                <div className="text-sm font-medium text-gray-500 dark:text-slate-400 uppercase">{format(day, 'EEE')}</div>
                <div className={clsx("text-xl font-bold mt-0.5", isToday ? "text-indigo-600" : "text-gray-900 dark:text-slate-100")}>
                    {format(day, 'd')}
                </div>
                {totalHours > 0 && (
                    <div className="text-[10px] text-gray-400 dark:text-slate-500 mt-1 font-medium">{totalHours}h Planned</div>
                )}
            </div>
